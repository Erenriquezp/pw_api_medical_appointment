...
public class PacienteService {
   ...
   @Transactional
   public void create(PacienteRepresentation patientR) {
      this.patientRepository.persist(this.mapperToPatient(patientR));
   }

   @Transactional
   public void update(Long id, PacienteRepresentation patientR) {
      Paciente patient = this.patientRepository.findById(id);
      if (patient != null) {
         patient.nombre = patientR.nombre;
         patient.appellido = patientR.apellido;
   ...
   @Transactional
   public void partialUpdate(Long id, PacienteRepresentation patientR) {
      Paciente patient = this.patientRepository.findById(id);
      if (patient != null) {
   ...
   private PacienteRepresentation mapper(Paciente patient) {
      ...
      patientR.apellido = patient.appellido;
